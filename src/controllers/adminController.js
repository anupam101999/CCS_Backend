...
const getStats = async (req, res) => {
  try {
    const [users, appts, tickets] = await Promise.all([
      pool.query(`SELECT COUNT(*) FROM users WHERE isadmin = FALSE`),
      pool.query(`SELECT status, COUNT(*) FROM appointment_bookings GROUP BY status`),
      pool.query(`SELECT status, COUNT(*) FROM notification_tickets GROUP BY status`),
    ]);
